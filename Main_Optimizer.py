...
def optPort(cov, G, mu=None):
    ...
    return w  
#----------------------------------------------------
def All_metrics(cov, G, weights, mu, MaxSharpe, Marketbetas, MarketMean):

    # Calculate MarketBeta
    MarketBeta = np.sum(np.multiply(Marketbetas, weights))
    # Treynor Metric
    TreynorRatio=(np.sum(np.multiply(weights,mu)))/MarketBeta

    # Alpha Metric
    Ri = np.sum(np.multiply(weights,mu))
    if (mu[0]==1):
        MarketMean=1
    AlphaRatio=Ri-(MarketMean*MarketBeta)

    Metrics_array = np.array([MaxSharpe,TreynorRatio,AlphaRatio])
    return(Metrics_array)  
#------------------------------------------------------
def calculations_IN(cov, mu, Marketbetas, MarketMean):
    points = 100
    ...
    for i in range(points):
        ...
        if (VAR_in[i]<=initial_min):
            ...
            MinVar_id = i
            weights_minvar = weights
            sharpe_minvar = Sharpe_temp
        if (Sharpe_temp>MaxSharpe):
            MaxSharpe = Sharpe_temp
            MaxSharpe_id = i
    Metrics_array=All_metrics(cov,G,weights_minvar,mu, sharpe_minvar, Marketbetas, MarketMean)
    return(VAR_in,MaxSharpe, MaxSharpe_id,MinVar_id, Metrics_array)                
#----------------------------------------------------
def calculations_OUT(covIn,covOut, mu, Marketbetas, MarketMean):
    points = 100
    G = np.linspace(0.01,100,points)
    ...
    for i in range(points):
        ...
        if (Sharpe_temp>MaxSharpe):
            MaxSharpe = Sharpe_temp
            MaxSharpe_id = i
    Metrics_array=All_metrics(covOut,G,weights_minvar,mu, sharpe_minvar, Marketbetas, MarketMean)
...
def frontier_graph(Cov_in1, Cov_in2, Cov_out1, Cov_out2, 
                         name,name2, Marketbetas, MarketMean, mu=None):
    """
    ...
    ### IN-SAMPLE
    #clean    
    VAR_in1, MaxSharpe, MaxSharpe_id, MinVar_id, Metrics_tab[0] = calculations_IN(Cov_in1, mu, Marketbetas, MarketMean)          
    VAR_min.append(VAR_in1[MinVar_id])
    VAR_max.append(VAR_in1[MaxSharpe_id])
    G_id1.append(G[MinVar_id])
    G_id2.append(G[MaxSharpe_id])

    plt.scatter(VAR_in1, G, label=r'$\xi_i$ (in-sample)', alpha=0.5) #'Clean(in-sample)'                    
    #dirty    
    VAR_in2, MaxSharpe, MaxSharpe_id, MinVar_id, Metrics_tab[1] = calculations_IN(Cov_in2, mu, Marketbetas, MarketMean)
    VAR_min.append(VAR_in2[MinVar_id])
    VAR_max.append(VAR_in2[MaxSharpe_id])
    G_id1.append(G[MinVar_id])
    G_id2.append(G[MaxSharpe_id])
    plt.scatter(VAR_in2, G, label=r'$\lambda_i$ (in-sample)', alpha=0.5) #'Dirty(in-sample)'                    
    ### OUT-SAMPLE
    #clean    
    VAR_out1, MaxSharpe, MaxSharpe_id, MinVar_id, Not_Useful = calculations_OUT(Cov_in1,Cov_out1, mu, Marketbetas, MarketMean)               
    VAR_min.append(VAR_out1[MinVar_id])
    VAR_max.append(VAR_out1[MaxSharpe_id])
    G_id1.append(G[MinVar_id])
    G_id2.append(G[MaxSharpe_id])
    plt.scatter(VAR_out1, G, label=r'$\xi_i$ (out-sample)', alpha=0.5) #'Clean(out-sample)'                
    #dirty    
    VAR_out2, MaxSharpe, MaxSharpe_id, MinVar_id, Not_Useful = calculations_OUT(Cov_in2,Cov_out2, mu, Marketbetas, MarketMean)
    VAR_min.append(VAR_out2[MinVar_id])
    VAR_max.append(VAR_out2[MaxSharpe_id])
    G_id1.append(G[MinVar_id])
    G_id2.append(G[MaxSharpe_id])
    plt.scatter(VAR_out2, G, label=r'$\lambda_i$ (out-sample)', alpha=0.5) #'Dirty(out-sample)'                
    # plot specifications
    plt.ylim([0,100])
...
def frontier_graph_cluster(Risk_in1, Risk_in2, Risk_out1, Risk_out2, A, Covariances, Risk_T, w_total,mu, name,name2, Marketbetas, MarketMean):
    """
    ...
    G_id1.append(G[MinVar_id])
    G_id2.append(G[MaxSharpe_id])
    Metrics.append(All_metrics(Covariances[A-1],G,w_total[A-1,MaxSharpe_id],mu, MaxSharpe, Marketbetas, MarketMean))
    for B in range(len(G)):        
    ...
    G_id1.append(G[MinVar_id])
    G_id2.append(G[MaxSharpe_id])
    Metrics.append(All_metrics(Covariances[0],G,w_total[1,MaxSharpe_id],mu, MaxSharpe, Marketbetas, MarketMean))
    for B in range(len(G)):
...
#-------------------------------------------------------------------
# MAIN FUNCTION
def main(name, method, marketdir, descriptive, alpha=.995, p=100, nFact=50, double_q=2, qmax=4,portfolio="MinVar"):
    """
        Main function.
        Args:
            Name (string): Name of the database to use.
            Method (string): Determine if it's real or synthetic data.
            marketdir: Directory of the market index to use.
    ...
    DataSP500 = pd.read_csv(marketdir, header=0)
    JustSP500 = DataSP500.iloc[:,-1].copy()
    MarketVar = np.var(JustSP500)
    MarketMean = np.mean(JustSP500)
    temp = JustSP500[:200]
    SP500returns=np.array(temp)
    SP500returns=SP500returns.reshape([1,200])
    X_Market = np.append(x_synth_In[:,:200],SP500returns, axis=0) 
    MarketCov = np.cov(X_Market)
    Marketbetas = MarketCov[:-1,-1]/MarketVar
    ...
    Metrics_Cumulative = np.zeros([8,6])
    Metrics_cluster = np.zeros([8,3])
    ...
    # Markowitz Frontier
    VAR_[2], VAR_[0], VAR_[3], VAR_[1], Metrics_Cumulative[0:2,0:3] = frontier_graph(
        Cov1In, cov0In, Cov1Out, cov0Out, name,name2[0], Marketbetas, MarketMean)
    # HRP Frontier 
    Metrics_cluster[0:2] = frontier_graph_cluster(Risk_T[2], Risk_T[0], Risk_T[3], Risk_T[1], 3,
                           Covariances, Risk_T, w_total,mu, name,name2[0], Marketbetas, MarketMean)

    # Markowitz Frontier
    VAR_[4], VAR_[0], VAR_[5], VAR_[1], Metrics_Cumulative[2:4,0:3] = frontier_graph(
        Cov2In, cov0In, Cov2Out, cov0Out, name,name2[1], Marketbetas, MarketMean)
    # HRP Frontier 
    Metrics_cluster[2:4] = frontier_graph_cluster(Risk_T[4], Risk_T[0], Risk_T[5], Risk_T[1], 5,
                           Covariances, Risk_T, w_total,mu, name,name2[1], Marketbetas, MarketMean)

    # Markowitz Frontier
    VAR_[6], VAR_[0], VAR_[7], VAR_[1], Metrics_Cumulative[4:6,0:3] = frontier_graph(
        Cov3In, cov0In, Cov3Out, cov0Out, name,name2[2], Marketbetas, MarketMean)
    # HRP Frontier 
    Metrics_cluster[4:6] = frontier_graph_cluster(Risk_T[6], Risk_T[0], Risk_T[7], Risk_T[1], 7,
                           Covariances, Risk_T, w_total,mu, name,name2[2], Marketbetas, MarketMean)

    # Markowitz Frontier
    VAR_[8], VAR_[0], VAR_[9], VAR_[1], Metrics_Cumulative[6:8,0:3] = frontier_graph(
        Cov4In, cov0In, Cov4Out, cov0Out, name,name2[3], Marketbetas, MarketMean)
    # HRP Frontier 
    Metrics_cluster[6:8] = frontier_graph_cluster(Risk_T[8], Risk_T[0], Risk_T[9], Risk_T[1], 9,
                           Covariances, Risk_T, w_total,mu, name,name2[3], Marketbetas, MarketMean)
    ...
    for j in sequencerange:
        ...
        METRICSCORE[int(j/2)]=np.array([MSE,MAE, MSE_C,MAE_C, Frobenius])
    ...
    METRICSCORE.columns = ['EMC Marko','EMA Marko','EMC HRP','EMA HRP',
                           'Frobenius']
    METRICSCORE.index = ['Ninguna','Recorte','TW','Lineal',
                           'No-Lineal']
    METRICSCORE.to_csv(name + "_MetricasCaseras.csv")
    
    Metrics_cluster=pd.DataFrame(Metrics_cluster)   
    Metrics_Cumulative[:,3:6] = Metrics_cluster
    Metrics_Cumulative2 = Metrics_Cumulative[[0,1,2,4,6],:]
    Metrics_tab=pd.DataFrame(Metrics_Cumulative2)    
    Metrics_tab.columns=["Razón de Sharpe","Razón de Treynor","Razón de Alfa",
                         "Sharpe HRP","Treynor HRP", "Alfa HRP"]
...
marketdir = "Data_Storage\\ApproxMarketCovnorm.csv"
descriptive = False
name="Data_Storage\\Synth200_q2_50s\\Synthetic200_q2_50s_99"
method="Real"
Covariances, cov0, Risks = main(name, method, marketdir, \
                                descriptive, alpha=.99, p=200, nFact=50, double_q=4, portfolio="MinVar")
